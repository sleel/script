...
用法: $0 [-e] [-k N] [-h]
  -e   实际执行（默认 dry-run）
  -k N 保留最近 N 个内核版本（默认 ${KEEP_COUNT}，当前内核始终额外保留）
  -h   显示帮助
EOF
exit 0
}
...
[[ "$KEEP_COUNT" =~ ^[0-9]+$ ]] || { err "-k 参数必须是非负整数"; exit 1; }
[[ $EUID -eq 0 ]] || { err "请使用 root 权限运行本脚本"; exit 1; }

# 并发锁：防止重复运行；外部命令统一加 9>&- 关闭锁 fd，脚本被强杀也不留孤儿锁
LOCK_ACTIVE=false
...
backup_rollback_materials(){
  ...
  dpkg -l 9>&- | awk '$1=="ii"{print $2, $3}' | grep -E '^(pve-kernel|proxmox-kernel)-' \
...
echo
if [ "${#REMOVE_PKGS[@]}" -eq 0 ]; then action "无需删除的内核/Meta 包"
...
if [ "${#REMOVE_PKGS[@]}" -gt 0 ]; then
  ...
  CODE=$?
  set -e
  if [ "$CODE" -ne 0 ]; then err "模拟失败，请检查日志：$SIM"; cat "$SIM"; exit 1; fi
...
[ -d "${TEST_ROOT}/var/log/journal" ] && find "${TEST_ROOT}/var/log/journal" -type f -mtime +"${JOURNAL_AGE_DAYS}" -delete || true
