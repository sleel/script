#!/bin/sh
# WireGuard Watchdog for OpenWrt v1.3.0
...
# 有 timeout 命令则给外部调用加超时保护，没有则原样执行
TIMEOUT_CMD=""
command -v timeout >/dev/null 2>&1 && TIMEOUT_CMD="timeout ${CMD_TIMEOUT}"

log() { logger -t "wg-watchdog" "$1"; }

atomic_write() { # 原子写入，防止断电导致文件损坏
    local val="$1" file="$2"
...
# 从 "IP:PORT" 提取纯 IP（仅处理 IPv4，本脚本不支持 IPv6 对端）
extract_ip() {
    echo "$1" | sed -E 's/:[0-9]+$//'
}

# 公网 IP 合法性校验：格式 + 数值范围(0-255) + 排除私有/CGNAT/保留段
is_public_ip() {
...
# 断路器静默检查：提前到探测最前面，静默期内跳过全部 DNS/ping/重启动作
silence_ts=0
...
real_ip=$($TIMEOUT_CMD nslookup "$DDNS_DOMAIN" 223.5.5.5 2>/dev/null \
    | awk '/^Address/ {print $NF}' | grep -E '^[0-9]+\.' | tail -n1)
[ -z "$real_ip" ] && real_ip=$($TIMEOUT_CMD resolveip -4 -t 3 "$DDNS_DOMAIN" 2>/dev/null | head -n 1)
